...
void Graphics::DrawSpriteSubstitute(int x, int y, Color sub, const Surface& s, Color chroma)
{
	DrawSpriteSubstitute(x, y, sub, s.GetRect(), s, chroma);
}

void Graphics::DrawSpriteSubstitute(int x, int y, Color sub, RectI srcRect, const Surface& s, Color chroma)
{
	DrawSpriteSubstitute(x, y, sub, srcRect, GetScreenRect(), s, chroma);
}

void Graphics::DrawSpriteSubstitute(int x, int y, Color sub, RectI srcRect, const RectI& clipRect, const Surface& s, Color chroma)
{
	assert(srcRect.left >= 0);
	assert(srcRect.right <= s.GetWidth());
	assert(srcRect.top >= 0);
	...
	for (int sy = srcRect.top; sy < srcRect.bottom; sy++)
	{
		for (int sx = srcRect.left; sx < srcRect.right; sx++)
		{
			...
			if (srcPixel != chroma)
			{
				PutPixel(x + sx - srcRect.left, y + sy - srcRect.top, sub);
			}
		}
	}
}
